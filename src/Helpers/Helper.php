...
class Helper
{
	...
	public static function BuildSlackNotificationAttachment($op) {
		...
		$fields[trans('calendar::seat.staging_sys')] = $op->getStagingSystem() ? $op->getStagingSystem() : trans('calendar::seat.unknown');
		...
		return function ($attachment) use ($op, $url, $fields) {
			$attachment->title($op->title, $url)
			 	->fields($fields)
			 	->footer(trans('calendar::seat.created_by') . ' ' . $op->user->name . ' Can you +1, not sure or -1 to this fleet, using the reactions?')
			 	->markdown(['fields']);
		};
	}
